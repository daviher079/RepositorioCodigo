...
def get_losses_df(df, team_id):
    mask_type   = df['type'].isin(LOSS_TYPES)
    mask_takeon = (df['type'] == 'TakeOn') & (df['outcomeType'] == 'Unsuccessful')
    mask_pass   = (df['type'] == 'Pass')   & (df['outcomeType'] == 'Unsuccessful')
    mask = (df['teamId'] == team_id) & (mask_type | mask_takeon | mask_pass)
    return df[mask].copy()


def draw_loss_zones(ax, df, team_name, away_team_name, col, path_eff):
    ...
    cmap = LinearSegmentedColormap.from_list('loss_cmap', [bg_color, col], N=20)
    ...
    if is_away:
        ...
        ax.set_title(f"Zonas de pérdida - Equipo visitante ({total})", color=line_color,
                     fontsize=30, fontweight='bold', path_effects=path_eff)
    else:
        ax.text(0, -5, "Dirección ataque --->", color=col, size=15, ha='left', va='center')
        ax.set_title(f"Zonas de pérdida - Equipo local ({total})", color=line_color,
                     fontsize=30, fontweight='bold', path_effects=path_eff)
...
if __name__ == '__main__':
    ...
    hdf = get_losses_df(df, hteamID)
    adf = get_losses_df(df, ateamID)

    fig, axes = plt.subplots(1, 2, figsize=(24, 12))
    fig.patch.set_facecolor(bg_color)

    draw_loss_zones(axes[0], hdf, hteamName, ateamName, hcol, path_eff)
    draw_loss_zones(axes[1], adf, ateamName, ateamName, acol, path_eff)

    plt.tight_layout()

    output_img = PROCESSED_FILE.replace('_processed.csv', '_zonas_de_perdida.png')
